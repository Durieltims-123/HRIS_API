...
class applicantTest extends TestCase
{
    ...
    public function test_add_applicant(): void
    {
        ...
        $this->assertDatabaseHas("applicants", $applicantCheck);
    ...
    public function test_edit_applicant(): void
    {
        $formData = [
            ...
            "answers" => [
                ...
                [
                    "question_id" => "2",
                    "number" => "34b",
                    "question" => "b. within the fourth degree (for Local Government Unit - Career Employee)?",
                    "answer" => "false",
                    "details" => null
                ],
                [
        ...
        $instance = Applicant::where([["first_name", "Duriel"]])->first();

        $user = User::factory()->create();
        $this->assertCount(0, $user->tokens);
        $this->actingAs($user);
        $this->patch("/api/applicant/" . $instance->id, $formData);
    ...
    public function test_delete_applicant(): void
    {
        ...
        $data = Applicant::where("first_name", "Durieltims")->first();
        $user = User::factory()->create();
        $this->assertCount(0, $user->tokens);
        $this->actingAs($user);
        $this->delete("/api/applicant/" . $data->id);
        // this will check if it is inserted in the database
        $this->assertDatabaseMissing("applicants", $applicantCheck);
    }
}
